...
| 7-15 | **R8 bei Files/Kamera/Galerie aus** | 2026-09-04 nach 7-14 nachgeholt: `optimization { enable = true }` + `isShrinkResources = true` für alle drei Apps aktiviert (identisches Muster zu Warden), keine App-eigenen Keep-Regeln nötig — Coil, Media3, OkHttp, CameraX (inkl. Extensions) und ZXing bringen ihre consumer-rules.pro selbst mit. Alle drei Release-Builds mit einem Wegwerf-Test-Keystore gebaut und auf dem physischen Testgerät installiert (kein Device-Owner-Risiko wie bei Warden, da normale Apps). Durchgetestet: Files (Speicherzugriff-Onboarding, Ordner-Navigation), Kamera (Kamera+Mikrofon-Onboarding, Testaufnahme über den vollen CameraX-Pfad, QR-/Barcode-Scanner über ZXing), Galerie (Medienzugriff-Onboarding, Album-Grid mit Paging, Vollbild-Viewer mit Coil/Media3) — alles ohne Absturz, keine `FATAL EXCEPTION` im Logcat. Die Kamera-Release-APK schrumpfte dabei von 18 MB auf 3,1 MB (Files 2,7 MB, Galerie 2,8 MB) |
| 7-16 | **Warden-Provisionierung nicht ausrollbar dokumentiert** | `warden/README.md` nannte nur `adb dpm set-device-owner` und den Satz „alternativ läuft die Einrichtung über QR-Provisionierung" — ohne QR-JSON, ohne `PROVISIONING_DEVICE_ADMIN_SIGNATURE_CHECKSUM` und ohne gehostete Download-URL. Jetzt gibt es beides: `warden/scripts/qr-provisioning.sh` erzeugt das Provisionierungs-JSON aus einer beliebigen signierten Release-APK (liest den Zertifikat-Fingerabdruck per `apksigner`/`keytool`, kodiert ihn Base64url ohne Padding — bewusst nicht der veraltete SHA-1-Ganzdatei-Checksum), und `README.md`s „Device Owner einrichten"-Abschnitt trägt jetzt den vollen Ablauf (6-Tipp-Auslöser auf dem Willkommensbildschirm, Skript-Aufruf mit Beispiel, Erklärung des Checksum-Feldes) samt dem 7-14-Fund als Warnhinweis (Rückzug von Device Owner nur per Werksreset, unabhängig vom Provisionierungsweg). Das Skript wurde gegen eine echte, test-signierte Release-APK verifiziert: erzeugter Fingerabdruck stimmt exakt mit `apksigner verify --print-certs` überein, das erzeugte JSON übersteht einen vollen QR-Kodier-/Dekodier-Rundlauf (Python `qrcode` → OpenCV `cv2.QRCodeDetector`) byteidentisch. Der eigentliche Geräte-Test (Werksreset, 6-Tipp-Geste, echter QR-Scan während der Ersteinrichtung) steht noch aus — er würde einen weiteren Werksreset des gerade erst wiederhergestellten Testgeräts brauchen, s. offene Frage an den Nutzer |
