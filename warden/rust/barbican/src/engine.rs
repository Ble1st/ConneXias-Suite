...
//!
//! **ChildVPN-Zweig (2026-08-31, `docs/design-barbican-prozess-childvpn.md`):** `run_engine_loop`
//! prüft `childvpn::is_child_vpn_armed()` als Allererstes in jeder Iteration. Ist ChildVPN scharf
//! geschaltet, überspringt der Tick den gesamten smoltcp-/NAT-/DNS-Filter-Pfad unten vollständig
//! und delegiert stattdessen an [childvpn::drive_tick] — reines WireGuard-Passthrough zur eigenen
//! VPS, kein selektives Routing einzelner Flows (das war eine frühere, vom Nutzer explizit
//! korrigierte Planvariante). Ein `clear_child_vpn_config()`-Aufruf lässt den nächsten Tick nahtlos
//! zurück in diesen Direct-Mode-Pfad fallen, ohne den Tunnel selbst neu aufzubauen.
...
/// Poll-Timeout des Lese-Threads in `start_captured_tunnel` (Millisekunden). Bestimmt die
/// Obergrenze, wie lange ein Thread nach einem `stop_captured_tunnel()`-Aufruf noch aktiv bleiben
/// kann, bevor er `reader_stop` erneut prüft und beendet — s. Kommentar am Lese-Thread für die
/// volle Begründung (Ersatz für unbegrenzt blockierendes `File::read()`). 200ms ist ein
/// Kompromiss: kurz genug, um disarm/rearm subjektiv sofort wirken zu lassen, lang genug, um bei
/// Leerlauf nicht spürbar CPU zu verbrauchen (Wakeup alle 200ms pro aktivem Tunnel).
const READER_POLL_TIMEOUT_MS: u16 = 200;
...
impl EngineHandle {
    fn stop(&mut self) {
        ...
        // Bis 2026-08-31: der Lese-Thread hing in einem unbegrenzt blockierenden `read()` und
        // wachte planmäßig erst auf, wenn Kotlin den `tun_fd` schloss/ersetzte — bei Always-On+
        // Lockdown-VPN (Android startet `WardenVpnService` nach `stopSelf()` fast sofort neu)
        // konnte der neue Tunnel exakt dieselbe fd-Zahl erhalten, sodass der alte Thread nie ein
        // Schließen/einen Fehler sah und dieser `join()` faktisch unbegrenzt blockierte —
        // live bestätigt als Ursache geleakter Threads samt Traffic-Verlust, s. Kommentar am
        // Lese-Thread in `start_captured_tunnel`. Seit dem dortigen Wechsel auf einen
        // `poll()`-basierten Lese-Loop mit `READER_POLL_TIMEOUT_MS` kehrt dieser `join()`
        // spätestens nach diesem Timeout zurück, unabhängig vom fd-Zustand. (`sinkhole.rs::
        // stop_sinkhole` hat dieselbe historische Einschränkung noch unverändert — dort separat
        // zu beheben, falls dort ebenfalls ein Always-On-Restart-Zyklus relevant wird.)
...
pub fn start_captured_tunnel(
    ...
    // ist die von `WardenVpnService.addDnsServer(...)` vergebene "DNS-Server"-Adresse — jede
    // Anfrage dorthin wird beim NAT-Relay auf `upstream_dns` umgeschrieben. Seit dem
    // ChildVPN-Fix vom 2026-08-31 (s. `WardenVpnService.UPSTREAM_DNS_IPV4`-Doc) übergibt Kotlin
    // hier bewusst denselben Wert wie `upstream_dns` — der Rewrite unten wird dadurch für
    // Direct-Mode zum reinen No-Op (`dst_addr == dns_sentinel` trifft sofort zu), die
    // Parametertrennung bleibt aber bestehen: ein rein interner, außerhalb des Tunnels nicht
    // erreichbarer Sentinel-Wert bliebe technisch weiterhin möglich, wird nur aktuell nicht mehr
    // genutzt (s. Kotlin-Doc für die volle Begründung, insbesondere warum ChildVPN diesen
    // Rewrite-Schritt strukturell nie durchläuft).
    let dns_sentinel =
    ...
    let reader_handle = thread::Builder::new()
        ...
        .spawn(move || {
            ...
            while reader_stop.load(Ordering::SeqCst) {
                // KRITISCH (2026-08-31, Root-Ursache für geleakte Reader-Threads nach
                // disarm/rearm unter Always-On+Lockdown-VPN): früher stand hier ein unbegrenzt
                // blockierendes `file.read(&mut buffer)`. Der Thread wachte planmäßig erst auf,
                // wenn Kotlin den `tun_fd` schloss (s. `EngineHandle::stop()`-Kommentar) — aber
                // Android startet `WardenVpnService` nach `stopSelf()` fast sofort neu und der
                // NEUE Tunnel bekam wiederholt exakt dieselbe fd-Zahl zugewiesen (Linux vergibt
                // geschlossene fds bevorzugt erneut, live beobachtet: immer `fd=99`). Kam der
                // Join-Aufruf in `EngineHandle::stop()` diesem Wechsel zeitlich zuvor, blieb der
                // alte Thread auf dem inzwischen dem NEUEN Tunnel gehörenden fd blockiert — der
                // liefert ihm ja weiterhin Daten, nur eben nicht das erwartete Schließen/den
                // erwarteten Fehler, der die Schleife hätte verlassen lassen. Ergebnis: der Thread
                // lief unbegrenzt weiter (live bestätigt: 3 statt der erwarteten 2
                // `warden-barbican`-Threads nach einem einzigen kontrollierten Zyklus) und
                // konkurrierte mit dem Lese-Thread des neuen Tunnels um dieselben eingehenden
                // Pakete auf demselben fd — plausible Erklärung für den zusätzlich beobachteten
                // Traffic-Verlust ("nichts hat Internet" bei aktivem ChildVPN). Fix: statt blind
                // zu lesen, zuerst mit kurzem Timeout pollen (`READER_POLL_TIMEOUT_MS`) — dadurch
                // wird `reader_stop` regelmäßig erneut geprüft, unabhängig davon, ob der fd
                // jemals wieder Daten oder einen Fehler für DIESEN Thread liefert.
                let mut pollfds = [PollFd::new(file.as_fd(), PollFlags::POLLIN)];
                match poll(&mut pollfds, READER_POLL_TIMEOUT_MS) {
                    Ok(0) => continue, // Timeout: reader_stop erneut prüfen, kein read() nötig
                    Ok(_) => {}
...
pub fn stop_captured_tunnel() {
    // KRITISCH (2026-08-31 Debugging-Session, s. CLAUDE.md "Netz-Sperre" — vermutlich die
    // tatsächliche Ursache des dort dokumentierten "RX-Freeze-Bugs"): `guard.take()` MUSS den
    // `MutexGuard` selbst noch VOR dem blockierenden `handle.stop()` fallen lassen, nicht erst am
    // Ende der Funktion. `EngineHandle::stop()` joint den Lese-Thread, der in einem blockierenden
    // `File::read()` hängt und typischerweise erst aufwacht, wenn Kotlin den `tun_fd` schließt
    // (s. `EngineHandle::stop()`-Kommentar) — dieses Schließen und das nachfolgende Öffnen eines
    // NEUEN Tunnels (bei Always-On+Lockdown startet Android `WardenVpnService` nach einem
    // `stopSelf()` fast sofort neu, um die Always-On-Garantie zu halten) können sich denselben
    // fd-Zahlenwert teilen (Linux vergibt geschlossene fds bevorzugt erneut) — läuft der alte
    // Lese-Thread dem Schließen zeitlich hinterher, blockiert sein `read()` ggf. beliebig lange auf
    // dem inzwischen dem NEUEN Tunnel gehörenden fd, statt mit einem Fehler zurückzukehren. Hielte
    // diese Funktion die `ENGINE`-Sperre während dieses Joins weiter (die vorherige Version: der
    // `if let ... && let Some(mut handle) = guard.take()`-Guard blieb bis Funktionsende in Scope),
    // würde JEDER nachfolgende `start_captured_tunnel()`-Aufruf – der dieselbe Sperre als
    // allererstes braucht – unbegrenzt blockieren, exakt das live beobachtete Symptom ("Calling
    // startCapturedTunnel(...)" erscheint in Logcat, das erwartete "returned OK" danach nie). Die
    // Sperre jetzt nur so kurz halten wie zum reinen `Option`-Tausch nötig, behebt diesen Deadlock
    // strukturell, unabhängig davon, ob der zugrunde liegende fd-Wiederverwendungs-Race selbst noch
    // behoben wird (separates, noch offenes Problem — s. Kommentar an `EngineHandle::stop()`).
    let handle = ENGINE.lock().ok().and_then(|mut guard| guard.take());
...
fn run_engine_loop(
    ...
    while stop.load(Ordering::SeqCst) {
        // ChildVPN-Zweig: s. Moduldoc "ChildVPN-Zweig" — komplett getrennter Pfad, kein Zugriff auf
        // smoltcp/`iface`/`sockets`/NAT unten, solange scharf geschaltet.
        if childvpn::is_child_vpn_armed() {
            match rx.recv_timeout(ENGINE_TICK) {
                Ok(packet) => childvpn::drive_tick(Some(&packet), &mut device.writer),
                Err(RecvTimeoutError::Timeout) => childvpn::drive_tick(None, &mut device.writer),
...
fn pump_established_sessions(
    ...
    for (key, handle) in flow_sockets.iter() {
        // KRITISCH (2026-08-31, Root-Ursache des historischen "RX-Freeze-Bugs" — s. CLAUDE.md
        // "Netz-Sperre"): `flow_sockets` ist die gemeinsame Map für TCP- UND UDP-Flows
        // (`pump_udp_listeners` trägt eigene, auf einen dauerhaften `udp::Socket`-Listener
        // zeigende Einträge in dieselbe Map ein, s. dortiger Kommentar). Ohne diesen Filter griff
        // die Schleife hier unten mit `sockets.get_mut::<tcp::Socket>(*handle)` unterschiedslos
        // auch auf UDP-Handles zu — smoltcps `SocketSet::get_mut` panict dann mit "handle refers
        // to a socket of a wrong type" (live bestätigt per Panic-Hook: sobald der allererste
        // UDP-Flow abgeschlossen war — typischerweise eine ganz gewöhnliche DNS-Anfrage, die
        // Sekunden nach Tunnelstart eintrifft), was den gesamten Engine-Thread unwinden ließ. Der
        // Lese-Thread lief unbemerkt weiter (erklärt das ebenfalls beobachtete "TX klettert / RX
        // komplett eingefroren / nur 1 statt 2 `warden-barbican`-Threads"-Muster), aber ohne
        // Engine-Thread verarbeitete `run_engine_loop` nie wieder ein Paket — weder den
        // Direct-Mode-NAT-Pfad noch (da `childvpn::drive_tick` aus demselben Thread heraus
        // angestoßen wird) den ChildVPN-Zweig, selbst wenn dessen Transport-Socket über
        // `spawn_transport_connect` unabhängig vom Engine-Thread bereits erfolgreich stand ("laut
        // Kotlin-Log verbunden, aber kein Handshake/kein Traffic" — der ChildVPN-Symptombericht,
        // der diese Debugging-Session ausgelöst hat).
        if !matches!(key.proto, Proto::Tcp) {
            continue;
        }
