...
class WardenApplication : Application() {
    ...
    override fun onCreate() {
        super.onCreate()
        // Prozess-Split "Barbican als eigener Prozess" (2026-08-31, Design-Dok
        // docs/design-barbican-prozess-childvpn.md): Application.onCreate() läuft in JEDEM Prozess
        // der App einmal, auch im neuen `:barbican`-Prozess (s. Manifest, WardenVpnService
        // android:process). Alles unterhalb ist Hauptprozess-Zustand — WardenLockSession/
        // ProcessLifecycleOwner-Beobachtung, WorkManager-Scheduling, Broadcast-Registrierung.
        // WardenVpnService/BarbicanEngine brauchen nichts davon: sie lesen ihre Config direkt per
        // EnvelopeFile (s. NetLockdownController-Klassendoc "kein Cross-APK-startService-Aufruf
        // mehr nötig") und werden über Intents gesteuert, nicht über diese Application-Instanz.
        // Ohne diesen Guard liefe der komplette Block hier ein zweites Mal — mit einem zweiten,
        // unabhängigen ProcessLifecycleOwner/WardenLockSession (der im :barbican-Prozess mangels
        // eigener Activities ohnehin nie sinnvoll feuert) und doppeltem, wenn auch idempotentem,
        // WorkManager-Scheduling bei jedem :barbican-Prozessstart.
        if (Process.myProcessName() != packageName) return
